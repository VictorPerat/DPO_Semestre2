...
/**
 * Campo de contraseña redondeado con placeholder y borde personalizado.
 */
public class RoundedPasswordField extends JPasswordField {
    ...
    protected void paintComponent(Graphics graphicsParameterValue) {
        ...
        g2LocalVariableValue.fillRoundRect(0, 0, getWidth(), getHeight(), radiusFieldReference, radiusFieldReference);

        g2LocalVariableValue.dispose();

        super.paintComponent(graphicsParameterValue);

        if (getPassword().length == 0 && !isFocusOwner() && placeholderFieldReference != null && !placeholderFieldReference.isEmpty()) {
            Graphics2D placeholderGraphicsLocalVariableValue = (Graphics2D) graphicsParameterValue.create();
            placeholderGraphicsLocalVariableValue.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
            placeholderGraphicsLocalVariableValue.setFont(getFont());
            placeholderGraphicsLocalVariableValue.setColor(new Color(164, 172, 186));

            Insets insetsLocalVariableValue = getInsets();
            FontMetrics fontMetricsLocalVariableValue = placeholderGraphicsLocalVariableValue.getFontMetrics();
            int textYLocalVariableValue = ((getHeight() - fontMetricsLocalVariableValue.getHeight()) / 2) + fontMetricsLocalVariableValue.getAscent();
    ...
    protected void paintBorder(Graphics graphicsParameterValue) {
        ...
        g2LocalVariableValue.setColor(hasFocus() ? focusBorderColorFieldReference : borderColorFieldReference);
        g2LocalVariableValue.drawRoundRect(0, 0, getWidth() - 1, getHeight() - 1, radiusFieldReference, radiusFieldReference);
