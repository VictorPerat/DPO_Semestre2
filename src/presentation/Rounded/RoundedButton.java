...
/**
 * Botón redondeado con soporte para degradado y modo outline.
 */
public class RoundedButton extends JButton {
    ...
    public void setGradientColors(Color startColorParameterValue, Color endColorParameterValue) {
    ...
    public void setOutlineMode(Color outlineColorParameterValue, int outlineThicknessParameterValue) {
    ...
    protected void paintComponent(Graphics graphicsParameterValue) {
        ...
        g2LocalVariableValue.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, RenderingHints.VALUE_TEXT_ANTIALIAS_ON);

        ButtonModel buttonModelLocalVariableValue = getModel();

        int shadowOffsetLocalVariableValue = (!outlineModeFieldReference && shadowEnabledFieldReference) ? 6 : 0;
        int drawHeightLocalVariableValue = getHeight() - shadowOffsetLocalVariableValue;
        ...
        if (outlineModeFieldReference) {
            Color fillColorLocalVariableValue = getBackground() != null
                    ? new Color(getBackground().getRed(), getBackground().getGreen(), getBackground().getBlue(), Math.max(25, getBackground().getAlpha()))
            ...
            g2LocalVariableValue.fillRoundRect(0, 0, getWidth() - 1, drawHeightLocalVariableValue - 1, radiusFieldReference, radiusFieldReference);

            g2LocalVariableValue.setStroke(new BasicStroke(outlineThicknessFieldReference));
            g2LocalVariableValue.setColor(buttonModelLocalVariableValue.isRollover()
            ...
            Color topColorLocalVariableValue = startColorFieldReference != null ? startColorFieldReference : getBackground();
            Color bottomColorLocalVariableValue = endColorFieldReference != null ? endColorFieldReference : getBackground().darker();

            if (buttonModelLocalVariableValue.isPressed()) {
                topColorLocalVariableValue = topColorLocalVariableValue.darker();
                bottomColorLocalVariableValue = bottomColorLocalVariableValue.darker();
            } else if (buttonModelLocalVariableValue.isRollover()) {
                topColorLocalVariableValue = shiftColor(topColorLocalVariableValue, 18);
                bottomColorLocalVariableValue = shiftColor(bottomColorLocalVariableValue, 10);
            }
        ...
        FontMetrics fontMetricsLocalVariableValue = g2LocalVariableValue.getFontMetrics();
        int textXLocalVariableValue = (getWidth() - fontMetricsLocalVariableValue.stringWidth(getText())) / 2;
        int textYLocalVariableValue = ((drawHeightLocalVariableValue - fontMetricsLocalVariableValue.getHeight()) / 2) + fontMetricsLocalVariableValue.getAscent();

        g2LocalVariableValue.drawString(getText(), textXLocalVariableValue, textYLocalVariableValue);
    ...
    private Color shiftColor(Color baseColorParameterValue, int amountParameterValue) {
        int redLocalVariableValue = Math.min(255, baseColorParameterValue.getRed() + amountParameterValue);
        int greenLocalVariableValue = Math.min(255, baseColorParameterValue.getGreen() + amountParameterValue);
        int blueLocalVariableValue = Math.min(255, baseColorParameterValue.getBlue() + amountParameterValue);

        return new Color(redLocalVariableValue, greenLocalVariableValue, blueLocalVariableValue, baseColorParameterValue.getAlpha());
    }
}